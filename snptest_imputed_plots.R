...
models <- list(
  Model1 = list(
    pattern = "^chr[0-9]+_imputed_01_sex_maternal_age\\.snptest(\\.gz)?$",
    title = "SNPTEST imputed model 1: Sex + maternal age"
  ),
  Model2 = list(
    pattern = "^chr[0-9]+_imputed_02_weight_category_exact_ga\\.snptest(\\.gz)?$",
    title = "SNPTEST imputed model 2: Weight category + birth weight relative to gestational age"
  ),
  Model3 = list(
    pattern = "^chr[0-9]+_imputed_03_gd_pre_spb_latency\\.snptest(\\.gz)?$",
    title = "SNPTEST imputed model 3: Gestational diabetes/preeclampsia + SPB/PPROM + latency"
  ),
  Model4 = list(
    pattern = "^chr[0-9]+_imputed_04_mothers_age_nr_preg\\.snptest(\\.gz)?$",
    title = "SNPTEST imputed model 4: Mother's age + number of pregnancies"
  ),
  Model5 = list(
    pattern = "^chr[0-9]+_imputed_05_prev_preg_mother_ancestry\\.snptest(\\.gz)?$",
    title = "SNPTEST imputed model 5: Previous pregnancy complications + maternal ancestry"
  ),
  Model6 = list(
    pattern = "^chr[0-9]+_imputed_06_w_ga_weight_sex\\.snptest(\\.gz)?$",
    title = "SNPTEST imputed model 6: Gestational age + birth weight + sex"
  )
)
...
to_chr_clean <- function(chr_value, source_file = NULL) {
  ...
  if (!is.null(source_file)) {
    chr_from_file <- suppressWarnings(
      as.numeric(sub("^.*chr([0-9]+).*$", "\\1", as.character(source_file), ignore.case = TRUE))
    )
    chr_num[is.na(chr_num) & !is.na(chr_from_file)] <- chr_from_file
  }
...
find_header_line_text <- function(file) {
...
read_one_snptest_file <- function(file) {
  header_text <- find_header_line_text(file)
...
standardise_one_file <- function(df, source_file) {
  ...
  if (is.na(snp_col) || is.na(chr_col) || is.na(bp_col) || is.na(p_col)) {
    message("Available columns in ", basename(source_file), ":")
    message(paste(names(df), collapse = ", "))
    stop("Could not identify required SNPTEST columns.")
  }

  out <- data.frame(
    SNP = as.character(df[[snp_col]]),
    CHR = to_chr_clean(df[[chr_col]], basename(source_file)),
    BP  = to_numeric_clean(df[[bp_col]]),
    P   = to_numeric_clean(df[[p_col]]),
    stringsAsFactors = FALSE
  )

  keep <- !is.na(out$CHR) & out$CHR %in% 1:22 &
          !is.na(out$BP) & out$BP > 0 &
          !is.na(out$P) & out$P > 0 & out$P <= 1

  out <- out[keep, , drop = FALSE]
  out <- out[order(out$CHR, out$BP), , drop = FALSE]

  out
}

read_model_clean <- function(pattern, input_dir = ".") {
  ...
  clean_parts <- vector("list", length(files))

  for (i in seq_along(files)) {
    f <- files[i]
    message("    Reading ", basename(f), " (", i, "/", length(files), ")")

    raw <- read_one_snptest_file(f)
    clean <- standardise_one_file(raw, f)
    rm(raw)
    gc(FALSE)

    message("      Valid SNPs: ", nrow(clean))
    clean_parts[[i]] <- clean
  }

  df <- rbindlist(clean_parts, fill = TRUE)
  df <- as.data.frame(df)
  df <- df[order(df$CHR, df$BP), , drop = FALSE]

  if (nrow(df) == 0) {
    stop("No valid SNPTEST rows after filtering.")
  }
...
prepare_manhattan_data <- function(df) {
  chr_len <- tapply(df$BP, df$CHR, max, na.rm = TRUE)
  chr_order <- as.numeric(names(chr_len))
  chr_len <- chr_len[order(chr_order)]
  chr_order <- chr_order[order(chr_order)]

  offsets <- cumsum(chr_len) - chr_len
  names(offsets) <- names(chr_len)

  plot_df <- df
  plot_df$offset <- offsets[as.character(plot_df$CHR)]
  plot_df$BPcum <- plot_df$BP + plot_df$offset
  plot_df$logP <- -log10(plot_df$P)

  center <- tapply(plot_df$BPcum, plot_df$CHR, function(x) (min(x, na.rm = TRUE) + max(x, na.rm = TRUE)) / 2)
  axis_df <- data.frame(CHR = as.numeric(names(center)), center = as.numeric(center))
  axis_df <- axis_df[order(axis_df$CHR), , drop = FALSE]

  list(data = plot_df, axis = axis_df)
}

prepare_qq_data <- function(df) {
  qq_df <- df[!is.na(df$P) & df$P > 0 & df$P <= 1, , drop = FALSE]
  qq_df <- qq_df[order(qq_df$P), , drop = FALSE]

  if (nrow(qq_df) == 0) stop("No valid p-values for QQ plot")

  n <- nrow(qq_df)
  qq_df$expected <- -log10(ppoints(n))
  qq_df$observed <- -log10(qq_df$P)
...
make_manhattan_plot <- function(df, model_title, y_max_common = NULL) {
  ...
  ggplot(plot_df, aes(x = BPcum, y = logP)) +
    geom_point(aes(color = factor(CHR %% 2)), size = 0.45, alpha = 0.85, show.legend = FALSE) +
    scale_color_manual(values = c("grey35", "grey70")) +
    geom_hline(aes(yintercept = genomewide_line, linetype = "Genome-wide"), color = "red", linewidth = 0.45) +
    geom_hline(aes(yintercept = suggestive_line, linetype = "Suggestive"), color = "blue", linewidth = 0.45) +
    ...
    scale_x_continuous(breaks = axis_df$center, labels = axis_df$CHR, expand = expansion(mult = c(0.005, 0.01))) +
    scale_y_continuous(limits = c(0, y_max), breaks = seq(0, y_max, by = 2), expand = c(0, 0)) +
    labs(title = model_title, x = "Chromosome", y = expression(-log[10](italic(p))), linetype = NULL) +
...
make_qq_plot <- function(df, model_title, max_axis_common = NULL) {
  ...
  ggplot(qq_df, aes(x = expected, y = observed)) +
    ...
    labs(title = model_title, x = expression(Expected~~-log[10](italic(p))), y = expression(Observed~~-log[10](italic(p)))) +
    theme_classic(base_size = 11) +
    theme(
      plot.title = element_text(size = 11, face = "bold", hjust = 0.5, margin = margin(b = 7)),
      axis.title.x = element_text(size = 10, face = "bold", margin = margin(t = 6)),
...
save_plot_grid <- function(plot_list, filename, ncol = 2, width = 16, height = 18, dpi = 300) {
  if (length(plot_list) == 0) stop("No plots available to save: ", filename)

  nrow <- ceiling(length(plot_list) / ncol)

  png(filename = filename, width = width, height = height, units = "in", res = dpi, bg = "white")
  grid.newpage()
...
for (model_name in names(models)) {
  ...
  df <- tryCatch(
    read_model_clean(models[[model_name]]$pattern, input_dir),
    error = function(e) {
  ...
  message("  Total valid SNPs: ", nrow(df))
  message("  Minimum p-value: ", signif(min(df$P, na.rm = TRUE), 4))

  fwrite(df, file.path(output_dir, paste0(model_name, "_snptest_imputed_clean_for_plotting.tsv")), sep = "\t")
...
out_manhattan <- file.path(output_dir, "combined_snptest_imputed_manhattan.png")
out_qq <- file.path(output_dir, "combined_snptest_imputed_qq.png")

save_plot_grid(manhattan_plots, filename = out_manhattan, ncol = 2, width = 18, height = 18, dpi = 300)
message("Saved: ", out_manhattan)

save_plot_grid(qq_plots, filename = out_qq, ncol = 2, width = 14, height = 18, dpi = 300)
